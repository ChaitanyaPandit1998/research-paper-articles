...
- what none of this actually solves — typos, uneven quantization loss, RRF's blind spots, and doubled maintenance
...
7. [What this doesn't solve](#7-what-this-doesnt-solve)
8. [A worked example: where hybrid search wins](#8-a-worked-example-where-hybrid-search-wins)
...
Part 1 closed its anatomy section with a reality check — a vector database is a search mechanism, not a relevance guarantee. The techniques in this article deserve the same treatment: each closes a real gap, but each has its own blind spot.
...
- **Hybrid search doubles the maintenance surface, not just the query-time cost already noted in §6.** There are now two indexes — an inverted index and an ANN index — that both have to stay in sync with the live corpus. A document that gets updated and re-embedded but not re-tokenized (or vice versa) can silently drift so that its dense and sparse representations describe two different versions of the content, a staleness risk Part 1 flagged for a single index that's now doubled here.
...
- **None of it is a free lunch.** Typos still break sparse retrieval, quantization loss isn't uniform across embedding spaces, RRF can't tell a SKU lookup from a conversational query, and running two indexes means two things that can drift out of sync — stacking techniques closes gaps, it doesn't remove the need to evaluate the whole pipeline on real queries.
