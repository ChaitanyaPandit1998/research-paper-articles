...
**llama.cpp (March 2023).** Meta released the original LLaMA weights as a research-only download, runnable at the time only through a multi-gigabyte PyTorch/CUDA stack. Days later, Georgi Gerganov ported the model to plain C/C++ with no external dependencies, built on top of his own tensor library, `ggml`. The pitch was almost provocative in its narrowness: run a 7-billion-parameter language model on a MacBook, in reasonable time, on the CPU alone. That required two things PyTorch inference at the time didn't prioritize — aggressive weight quantization (packing each parameter into 4 bits instead of 16) and a runtime with none of the memory and dependency overhead of a full Python/CUDA stack. It worked. The project spawned an entire ecosystem — GGUF as a portable model format, `llama-cpp-python` bindings, Ollama built directly on top of it — and did more than any single project to make "run a real LLM on your own laptop" an ordinary thing to do rather than a research demo.

**vLLM (June 2023).** A team at UC Berkeley's Sky Computing Lab, led by Woosuk Kwon and Zhuohan Li, published *"Efficient Memory Management for Large Language Model Serving with PagedAttention"* (later presented at SOSP 2023) — the paper that introduced vLLM and diagnosed why existing GPU serving stacks left so much throughput on the table. The KV cache (the running memory of every previous token's attention keys and values) was being allocated in large contiguous chunks per request, the same way early operating systems allocated memory before virtual memory and paging existed. That led to the same disease: fragmentation and waste, in this case up to 60–80% of GPU memory sitting unused because it had been reserved for a worst-case sequence length that most requests never reached. vLLM's answer borrowed the OS solution directly — page the KV cache into small fixed-size blocks, the same way an operating system pages RAM, and let requests share and reclaim blocks dynamically instead of reserving a maximum up front.
...
Training produces a static set of weights. Turning those weights into something that answers a prompt is a genuinely different problem, with different bottlenecks entirely — none of it involves backpropagation, loss functions, or activation functions (see the companion [backpropagation](backpropagation.md), [loss functions](loss-functions.md), and [activation functions](activation-functions.md) articles for that earlier half of the model's life). Solving it means answering three questions, one at a time below.
...
**Generating one token at a time, fast.** Unlike training, where a whole batch of sequences is scored in parallel against known targets, autoregressive generation is inherently sequential: token $t{+}1$ can't be predicted until token $t$ has been sampled and fed back in. Every generated token still requires a full pass through every layer of the network — but because that pass is scoring one sequence, not a large batch, it's memory-bandwidth-bound rather than compute-bound: the GPU or CPU spends most of its time waiting to read weights from memory, not computing with them. This is precisely why quantization (shrinking how many bytes have to move) and batching (amortizing that memory traffic over many sequences at once) are the two levers every inference engine pulls.
...
llama.cpp built its entire identity around this trade. Its GGUF format ships models in a family of quantization schemes, computed once offline and baked into the file a user downloads: Q8_0 (8-bit, near-lossless) at one end, Q2_K (2-bit, aggressive and noticeably lossier) at the other, with Q4_K_M in between as the most commonly used default. Q4_K_M mixes precision per-tensor — spending more bits on the layers most sensitive to rounding, fewer on the ones that tolerate it. A 7B model that needs ~13.5GB at 16-bit precision needs roughly 4GB at Q4_K_M — the entire reason a MacBook with 16GB of unified memory can run it at all.
...
Pre-PagedAttention serving stacks allocate each sequence's KV cache as one contiguous block sized for a maximum sequence length — and llama.cpp's traditional design, built for a single interactive user rather than hundreds of concurrent strangers, works the same way. For a single local user, that waste is a rounding error against 16GB of RAM. For a GPU serving thousands of concurrent, wildly variable-length requests, it's a different story: Kwon et al. measured 60–80% of allocated KV-cache memory going unused this way.
...
llama.cpp's original design point was a single interactive session — one user, one conversation, request-response — and its architecture reflects that. `llama-server` has since added request batching, but it is not the axis the project was built to maximize.

vLLM's defining scheduling trick is **continuous batching** (also called iteration-level scheduling). Static batching waits for an entire batch of requests to finish generating before starting the next one, which stalls on whichever sequence in the batch happens to run longest. vLLM avoids that by re-evaluating the batch composition at every single decoding step, injecting newly arrived requests and evicting completed ones mid-stream. No GPU cycle is spent waiting on a straggler, and a request that arrives mid-stream doesn't wait for the entire current batch to drain first.
...
Because contiguous allocation reserves for the worst case every request *might* reach, it wastes roughly 75% of the memory it holds — matching the 60–80% figure the PagedAttention paper measured empirically across existing systems. vLLM's own paging brings that waste down to **under 4%**, and reclaims the difference directly into more concurrent capacity: a **4× increase in requests served from the same GPU memory**, with no change to the model, the hardware, or the quantization. That's also the structural reason behind vLLM's throughput numbers — the paper reports 2–4× higher throughput than FasterTransformer and Orca at the same latency — and it comes from memory accounting, not from a faster kernel.
...
**Ollama.** Not a competing engine — it's built directly on top of llama.cpp, using it as the inference backend under a friendlier CLI and model-management layer. It inherits llama.cpp's quantization-first, local-first design entirely; what it changes is the distribution and packaging story, not the underlying mechanism.

**MLC-LLM / Apple MLX.** Compiler-based approaches that generate hardware-specific inference kernels (for mobile GPUs, WebGPU, Apple Silicon) rather than shipping one general-purpose runtime. The goal is close in spirit to llama.cpp's "run anywhere with modest resources" — the difference is compiling per-target instead of shipping a single portable C++ codebase.
